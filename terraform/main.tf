...
resource "aws_ecs_cluster" "spartify" {
  name = "spartify"
}

resource "aws_ecr_repository" "spartify" {
  name                 = "spartify"
...
module "webserver" {
  ...
  ecr_repository = aws_ecr_repository.spartify.repository_url
  ecs_cluster    = aws_ecs_cluster.spartify.id
